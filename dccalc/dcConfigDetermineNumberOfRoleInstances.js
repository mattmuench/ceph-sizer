const dcConfigDetermineNumberOfRoleInstances = function (generalValuesLocal, workloadsArrayLocal, sizingConstraints, dcConfigArrayLocal) {
  ...
  for (let dcItem = 0; dcItem < generalValuesLocal.numberOfDCsPossible; dcItem++) {
    ...
    if (generalValuesLocal.globalDebug == true || localDebug == true) {
      console.log(`dcConfigDetermineNumberOfRoleInstances() 49: DC=${dcItem} => #scale-out role instances: ${dcConfigArrayLocal[dcItem].numberOfLocalScaleoutInstances}, #special role instances: ${dcConfigArrayLocal[dcItem].numberOfLocalSpecialInstances}`)
    }
  }
}
